class Facebook::SendOnFacebookService < Base::SendOnChannelService
  ...
  def fb_text_message_params
    params = {
      ...
      messaging_type: message_tag.present? ? 'MESSAGE_TAG' : 'RESPONSE'
    }
    params[:tag] = message_tag if message_tag.present?
    params
  end
  ...
  def message_tag
    @message_tag ||= GlobalConfigService.load('ENABLE_MESSENGER_CHANNEL_HUMAN_AGENT', nil) ? 'HUMAN_AGENT' : nil
  end
