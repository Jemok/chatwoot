# Banking demo (#8): platform-side moderation for FB Page comments.
# Calls Graph API to hide or delete a comment that originated from the page.
# When `MODERATION_SIMULATED=true` (or token lacks scopes), the platform call
# is skipped — local message state still updates and the action is audited.
class Facebook::ModerationService
  HIDE_SCOPE_REQUIRED = 'pages_manage_engagement'.freeze
  ...
  def initialize(message:, user:, action:)
    @message = message
    @user = user
    @action = action.to_s # 'hide' | 'unhide' | 'delete'
  end
  ...
  def graph_query
    base = { access_token: channel.try(:page_access_token) }
    return base if @action == 'delete'

    base.merge(is_hidden: @action == 'hide')
  end
  ...
  def log_audit(ok, simulated:)
    PolicyViolationLog.create!(
      ...
      action_attempted: "facebook.#{@action}",
      details: "Message ##{message.id} #{@action} on FB (success=#{ok}, simulated=#{simulated})#{@error ? " err=#{@error}" : ''}"
    )
  end
end
