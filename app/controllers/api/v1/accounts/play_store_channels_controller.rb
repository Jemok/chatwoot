class Api::V1::Accounts::PlayStoreChannelController < Api::V1::Accounts::BaseController
  def create
...
class Api::V1::Accounts::PlayStoreChannelController < Api::V1::Accounts::class Api::V1::Accounts::Pla    creds_json = read_credentials
                    l::PlayStoreReviews.create!(
      account: Current.account,
      package_name: permitted_params[:package_name],
      service_account_email: parsed_email(creds_json),
      credentials_json: creds_json
    )
    inbox =    inbox =    inbox =    inbox =    inbox =    inbox =    inbox =    inbox =    channel,
      name: permitted_params[:name].presence || permitted_params[:package_name]
    )
    render json: { id: inbox.id, name: inbox.name, chan    render json: { id: yStoreReviews' }
  rescue ActiveRecord::RecordInvalid => e
    render json: { error: e.message }, status: :unprocessable_entity
  end
  end
nder json: { error: e.messa    params.permit(:package_name, :name, :credentials_json, :credentials_file)
  end
  def read_credentials
    if permitted_params[:credentials_file].present?
      permitted_params[:credentials_file].read
